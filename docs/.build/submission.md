...
Gain-Error, Loss-Correct, and Loss-Error conditions. EEG was recorded and feedback-locked single-trial means were
...
committee guidelines (Keil et al., 2014).
...
. The assignment is a guaranteed bijection; the
...
: the MEQ score, attached by participant name,
...
. Participant-level features were
aggregated to one row per participant and
...
excluded as predictors to prevent leakage.
...
(Benjamini & Hochberg, 1995). Where the
...
**Machine-learning classification (Figure 5).** Chronotype was classified at the
...
![Figure 5. Leakage-safe, permutation-clean evaluation pipeline and out-of-fold classifier ROC.](docs/figures/fig_main_5_roc_pipeline.png)
...
agnostic **behavioural embedding** used downstream.
...
Daw, 2011) and, as a robustness check, refit in a Bayesian
**hierarchical partial-pooling** model with a chronotype group-level offset on each
parameter, sampled with NUTS (Ahn et al., 2017).
...
to form learned EEG embeddings.
...
MNE-Python, scikit-learn, PyMC), with fixed seed 0; see for
full detail.
...
### 3.4 Robustness across participant exclusions (Supplementary Figure S1)
...
### 3.5 Machine-learning classification of chronotype (Figure 5; see also Supplementary Figures S2 and S4)

Under nested cross-validation with hyperparameter tuning (Figure 5), the five
...
0.68; confusion matrix Supplementary Figure S4) with a pooled ROC AUC of 0.79 (Figure 5).
...
showed the same ordering (Supplementary Figure S2). The multivariate classifier thus relies on
...
### 3.6 Secondary: trial-level risky choice (Supplementary Figure S3)
...
### 3.7 Chronotype is decodable from risky-choice dynamics alone (Figure 3)

![Figure 3. Chronotype decoding from behavioural choice dynamics (GRU), from validated ERP contrasts, and their super-additive fusion.](docs/figures/fig_main_3_fusion.png)
...
### 3.8 Behaviour and neural feedback signals fuse super-additively (Figure 3; headline)
...
### 3.9 The continuous MEQ score is predictable (Figure 3)
...
### 3.10 A reinforcement-learning model localises the mechanism (Figure 3)
...
### 3.11 Honest negative: deep learning on single-trial EEG (Figure 3)
...
### 3.12 The chronotype effect is trait-level, not a single-trial coupling (Figure 4)

![Figure 4. Single-trial feedback P300 to next-trial risk coupling by chronotype (overall and valence-resolved).](docs/figures/fig_main_4_single_trial_coupling.png)
...
comparatively down-weights losses (cf. loss aversion; Kahneman & Tversky, 1979). That the frontocentral FRN did not
...
neural results stand independently of it.
